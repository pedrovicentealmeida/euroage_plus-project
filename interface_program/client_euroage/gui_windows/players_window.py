...
from custom_components.CustomTreeView import CustomTreeView
from network.client import SocketClientServer
from utils.helpers import PatientDataFetcher
from gui_windows.new_player_window import NewPlayerWindow
...
class PlayersWindow:
    ...
    def add_player(self):
        NewPlayerWindow(self.root, self.client)
        self.data = PatientDataFetcher(self.client).fetch_patient_data()  # Recarrega os dados
        self.tree_view.tree.delete(*self.tree_view.tree.get_children()) # Limpa a árvore
        self.tree_view.insert_data(self.data) # Atualiza a árvore

    def edit_player(self):
        NewPlayerWindow(self.root, self.client, self.tree_view.tree.item(self.tree_view.tree.focus(), 'values'))
        self.data = PatientDataFetcher(self.client).fetch_patient_data()  # Recarrega os dados
        self.tree_view.tree.delete(*self.tree_view.tree.get_children()) # Limpa a árvore
        self.tree_view.insert_data(self.data) # Atualiza a árvore
    ...
    def delete_player(self):
        ...
        if response:
            self.client.send_message_server("400")
            
            player_info = self.tree_view.tree.item(self.tree_view.tree.focus(), 'values')
            self.client.send_message_server(player_info[0])
