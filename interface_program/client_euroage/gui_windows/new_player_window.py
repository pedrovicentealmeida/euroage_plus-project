...
class NewPlayerWindow:
    ...
    def save_player(self):
        if self.player_info:
            self.client.send_message_server("300")
            self.client.send_message_server(self.player_info[0])
        else:
            self.client.send_message_server("200")

        self.client.send_message_server(self.nome_text.get().strip())
        self.client.send_message_server(self.idade_text.get().strip())
        self.client.send_message_server(self.profissao_text.get().strip())
        self.client.send_message_server(str(self.passatempos_text.get("1.0", "end-1c").strip()))
        self.client.send_message_server(str(self.mmse_scale.get()))

        deficit_mapping = {0: "Nulo", 1: "Leve", 2: "Moderado"}
        self.client.send_message_server(deficit_mapping[self.deficit_var.get()])
        ...
        self.client.send_message_server(nomes_relacoes_str)

        data = self.client.receive_message_server() # Recebe ACK
