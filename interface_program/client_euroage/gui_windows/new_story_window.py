...
from gui_windows.telling_story_window import TellingStoryWindow
...
class NewStoryWindow:
    ...
    def create_add_button(self, parent):
        ...
        adicionar.grid(row=0, column=1, padx=10)

    def create_search_entry(self, parent):
        frame = LabelFrame(parent, padx=5, pady=5)
        frame.grid(row=0, column=0)

        self.root.search_img = PhotoImage(file="icons/search.png")
        search_label = Label(frame, image=self.root.search_img)
        search_label.grid(row=0, column=0, padx=5)

        self.search_var = StringVar()
        self.search_var.set("Pesquisa...")
        self.search = Entry(frame, font=("Consolas", 11), borderwidth=2, textvariable=self.search_var)
    ...
    def add_player(self):
        NewPlayerWindow(self.root, self.server)
        self.data = PatientDataFetcher(self.server).fetch_patient_data()
        self.tree_view.tree.delete(*self.tree_view.tree.get_children()) 
        self.tree_view.insert_data(self.data)